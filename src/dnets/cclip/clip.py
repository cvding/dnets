...
class CLIP(nn.Module):
    ...
    def encode_text(self, text_ids_tensor, text_mask_tensor):
        x = self.bert(text_ids_tensor, attention_mask=text_mask_tensor)[0].type(self.dtype) # [batch_size, seq_length, hidden_size]
        return x[:, 0, :] @ self.text_projection

    def forward(self, image, text):
        assert image is not None and text is not None, "text and image both cannot be None!"
